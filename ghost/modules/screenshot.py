...
class GhostModule(Module):
    ...
    def run(self, argc, argv):
        ...
        self.device.send_command("screencap /tmp/screenshot.png")

        exists, file = self.fs.exists_directory(argv[0])
        if exists and file == 'file':
            if self.device.download('/tmp/screenshot.png', argv[0]):
                self.print_success("Screenshot has been downloaded!")

        if exists and file == 'directory':
            if self.device.download('/tmp/screenshot.png', argv[0] + '/screenshot.png'):
                self.print_success("Screenshot has been downloaded!")
                
        self.device.send_command("rm /tmp/screenshot.png")
